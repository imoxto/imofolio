...
export const getStaticProps: GetStaticProps<BlogPostProps, PathProps> = async ({ params }) => {
	const post = (await getAllPostSlugs()).find((p) => p.slug === params.slug);
	const { frontmatter, source } = await getPost(post.path);
...
export default function BlogPost({ post }: BlogPostProps): JSX.Element {
	return (
		<>
			<Layout.Blog
				seo={{
					title: `${post.frontmatter.title} ─ blog ─ ${profile.alias}`,
